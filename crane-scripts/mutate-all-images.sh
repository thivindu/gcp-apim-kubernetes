#!/usr/bin/env bash
set -euo pipefail

# Shared annotation for all images
ANNOTATION_KEY="com.googleapis.cloudmarketplace.product.service.name"
ANNOTATION_VALUE="services/wso2-apim-apk.endpoints.wso2-marketplace-public.cloud.goog"

# Define all images to annotate
declare -a IMAGES=(
  "us-docker.pkg.dev/wso2-marketplace-public/wso2-marketplace/wso2am-acp:4.5"
  "us-docker.pkg.dev/wso2-marketplace-public/wso2-marketplace/apk-config-deployer-service:4.5"
  "us-docker.pkg.dev/wso2-marketplace-public/wso2-marketplace/apk-adapter:4.5"
  "us-docker.pkg.dev/wso2-marketplace-public/wso2-marketplace/apk-common-controller:4.5"
  "us-docker.pkg.dev/wso2-marketplace-public/wso2-marketplace/apk-ratelimiter:4.5"
  "us-docker.pkg.dev/wso2-marketplace-public/wso2-marketplace/apk-router:4.5"
  "us-docker.pkg.dev/wso2-marketplace-public/wso2-marketplace/apk-enforcer:4.5"
  "us-docker.pkg.dev/wso2-marketplace-public/wso2-marketplace/apk-idp-domain-service:4.5"
  "us-docker.pkg.dev/wso2-marketplace-public/wso2-marketplace/apk-idp-ui:4.5"
  "us-docker.pkg.dev/wso2-marketplace-public/wso2-marketplace/apim-apk-agent:4.5"
  "us-docker.pkg.dev/wso2-marketplace-public/wso2-marketplace/deployer:4.5"
)

# Check dependencies
for cmd in crane jq; do
  if ! command -v "$cmd" >/dev/null 2>&1; then
    echo "Error: '$cmd' is not installed or not on PATH." >&2
    exit 1
  fi
done

echo "======================================"
echo "Annotating all images with:"
echo "  ${ANNOTATION_KEY} = ${ANNOTATION_VALUE}"
echo "======================================"
echo

# Function to annotate a single image
annotate_image() {
  local IMAGE="$1"
  local IMAGE_REPO="${IMAGE%:*}"
  local TAG="${IMAGE##*:}"
  
  echo ""
  echo "=========================================="
  echo "Processing: ${IMAGE}"
  echo "=========================================="
  
  echo "Fetching manifest for ${IMAGE}..."
  if ! INDEX_JSON="$(crane manifest "${IMAGE}" 2>&1)"; then
    echo "Error: Failed to fetch manifest for ${IMAGE}" >&2
    echo "Skipping this image..." >&2
    return 1
  fi
  
  MEDIA_TYPE="$(printf '%s\n' "${INDEX_JSON}" | jq -r '.mediaType')"
  
  # Check if it's a multi-platform index
  if [ "${MEDIA_TYPE}" = "application/vnd.oci.image.index.v1+json" ] || \
     [ "${MEDIA_TYPE}" = "application/vnd.docker.distribution.manifest.list.v2+json" ]; then
    
    echo "Image is a multi-platform index. Processing per-platform manifests..."
    
    # Read digest and platform together
    MANIFEST_LINES="$(printf '%s\n' "${INDEX_JSON}" \
      | jq -r '.manifests[] | "\(.digest) \(.platform.os)/\(.platform.architecture)"')"
    
    if [ -z "${MANIFEST_LINES}" ]; then
      echo "Error: no manifests found in index for ${IMAGE}." >&2
      return 1
    fi
    
    TMP_TAGS=()
    
    # Tag and mutate each platform manifest
    echo "${MANIFEST_LINES}" | while read -r digest platform; do
      if [ -z "${digest}" ] || [ -z "${platform}" ]; then
        continue
      fi
      
      tmp_tag="${TAG}-${platform//\//-}-tmp"
      tmp_ref="${IMAGE_REPO}:${tmp_tag}"
      
      echo "  Processing platform ${platform}"
      echo "    Source digest: ${digest}"
      echo "    Temp tag:      ${tmp_ref}"
      
      # Tag digest with temp tag
      crane tag "${IMAGE_REPO}@${digest}" "${tmp_tag}"
      
      # Mutate annotation on the temp tag
      crane mutate \
        --annotation "${ANNOTATION_KEY}=${ANNOTATION_VALUE}" \
        -t "${tmp_ref}" \
        "${tmp_ref}"
      
      echo "    ✓ Annotation updated on ${tmp_ref}"
    done
    
    # Collect mutated temp tags from registry
    echo "  Collecting mutated temp tags from registry..."
    TMP_TAGS=()
    for ref in $(crane ls "${IMAGE_REPO}" | grep "^${TAG}-" || true); do
      TMP_TAGS+=("${IMAGE_REPO}:${ref}")
    done
    
    if [ "${#TMP_TAGS[@]}" -eq 0 ]; then
      echo "Error: no temp tags found after mutation for ${IMAGE}." >&2
      return 1
    fi
    
    echo "  Rebuilding multi-platform index on original tag: ${IMAGE}"
    APPEND_ARGS=()
    for t in "${TMP_TAGS[@]}"; do
      echo "    Including ${t} in new index"
      APPEND_ARGS+=("-m" "$t")
    done
    
    crane index append "${APPEND_ARGS[@]}" -t "${IMAGE}"
    
    echo "  ✓ Multi-platform index rebuilt successfully"
    
  else
    # Single-platform image
    echo "Image is a single-platform image (mediaType=${MEDIA_TYPE})"
    echo "Applying annotation directly..."
    
    crane mutate \
      --annotation "${ANNOTATION_KEY}=${ANNOTATION_VALUE}" \
      -t "${IMAGE}" \
      "${IMAGE}"
    
    echo "  ✓ Annotation updated on ${IMAGE}"
  fi
  
  echo "✓ Successfully processed ${IMAGE}"
  return 0
}

# Process all images
FAILED_IMAGES=()
SUCCESS_COUNT=0

for IMAGE in "${IMAGES[@]}"; do
  if annotate_image "${IMAGE}"; then
    ((SUCCESS_COUNT++))
  else
    FAILED_IMAGES+=("${IMAGE}")
  fi
done

# Summary
echo ""
echo "======================================"
echo "SUMMARY"
echo "======================================"
echo "Total images: ${#IMAGES[@]}"
echo "Successfully processed: ${SUCCESS_COUNT}"
echo "Failed: ${#FAILED_IMAGES[@]}"

if [ "${#FAILED_IMAGES[@]}" -gt 0 ]; then
  echo ""
  echo "Failed images:"
  for img in "${FAILED_IMAGES[@]}"; do
    echo "  - ${img}"
  done
  exit 1
else
  echo ""
  echo "✓ All images successfully annotated!"
fi
